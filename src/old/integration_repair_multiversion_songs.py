...
DIR_OUT = f'{DIR_BASE}{DIR_SEP}77_integrating_b'
...
class Song:
    ...
    def __eq__(self, other) -> bool:
        yes = (self.name == other.name)\
              and (self.artist == other.artist)\
              and (self.album == other.album) # remove if not doing strict
        
        # if yes and (self.album != other.album):
            # print(f'Old album: {self.album}\nNew album:{other.album}')
...
def run():
    ...
    get_artist_song_sets(DIR_OLD, old)
    multi_old = filter(lambda k: len(old[k]) > 1, old)
    multi_old_u = []
    for k in multi_old:
        multi_old_u.extend(old[k])

    get_songs(DIR_NEW, new)
    unacc = list(get_unaccounted(multi_old_u, new))
    
    print(f'n uac: {len(unacc)}')

    get_songs(DIR_OUT[:-2], inter)
    unacc = list(get_unaccounted(unacc, inter))
    
    print(f'n uac: {len(unacc)}')

    copy_unaccounted(unacc)
    

##    get_artist_song_sets(DIR_NEW, new)
##    multi_new = filter(lambda k: len(new[k]) > 1, new)
##    multi_new_u = []
##    for k in multi_new:
##        multi_new_u.extend(new[k])
##
##    print(f'multi new: {len(list(multi_new_u))}')
