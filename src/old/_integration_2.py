...
def get_songs(path: str, songs: dict={}) -> list:
    ...
    parent = from_parent(path, 1)
    for f in files:
        key = normalize(prepend_path(parent, f))
        location = prepend_path(path, f)
        pieces = path_pieces(path)[1:]
        pieces.reverse()

        songs.setdefault(key, []).append({location: pieces})
