...
def p_repeat_till_quit(action: callable, do_while: bool=True, c_phrase: str='continue', q_phrase: str='quit', c_key: str='', q_key: str='Q', indent: int=0):
    if do_while:
        print()
        action()
        print()
    ...
    prompt = f'Hit {c_key_print} to {c_phrase} or {q_key} to {q_phrase}'

    while True:
        choice = in_prompt(prompt, indent).upper()
        if choice == c_key.upper():
            print()
            action()
            print()
        elif choice == q_key.upper():
            print()
            break
